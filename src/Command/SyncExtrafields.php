...
use App\Entity\Extrafields;
use App\Entity\Extrafieldsvalues;
...
#[AsCommand(
    name: 'app:syncextrafields',
)]
class SyncExtrafields extends Command
{
    ...
    {
        $this->setName('SyncExtrafields')
            ->setDescription('Synchronistaion des extrafields')
            ->setHelp('');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($ListeConnector as $Val){
            ...
            $this->recupererListeExtrafield($OrigineId, $UrlApi, $TokenApi);
        }

        return Command::SUCCESS;
    }

    private function recupererListeExtrafield($OrigineId, $UrlApi, $TokenApi){
        ...
        $RequeteApi = $this->ClientHTTP->request("GET", $UrlApi."/setup/extrafields?sortfield=t.pos&sortorder=ASC");
        if($RequeteApi->getStatusCode() == 200){
            $content = json_decode($RequeteApi->getContent(), true);
            foreach($content as $Tab1=> $Tab2) {
                foreach($Tab2 as $Tab3=> $Tab4){
                    $ZoneExtrafield = $Tab1;
                    $CodeExtrafield = $Tab3;
                    $TypeExtrafield = $content[$Tab1][$Tab3]['type'];
                    $LabelExtrafield = $content[$Tab1][$Tab3]['label'];
                    $occurence = $em->getRepository(Extrafields::class)
                        ->findOneBy(array(
                            "zone" => $ZoneExtrafield,
                            "code" => $CodeExtrafield,
                            "origineid" => $OrigineId
                    ...
                    if(is_null($occurence)) {
                        $tp_entity = new Extrafields();
                        $tp_entity ->setCode($CodeExtrafield)
                                ->setLabel($LabelExtrafield)
                                ->setType($TypeExtrafield)
                                ->setZone($ZoneExtrafield)
                        ...
                        $occurence ->setCode($CodeExtrafield)
                            ->setLabel($LabelExtrafield)
                            ->setType($TypeExtrafield)
                            ->setZone($ZoneExtrafield)
                            ->setOrigineid($em->getReference('App\Entity\Myconnectors', $OrigineId));
                    ...
                    $occurence2 = $em->getRepository(Extrafields::class)
                        ->findOneBy(array(
                            "zone" => $ZoneExtrafield,
                            "code" => $CodeExtrafield,
                            "origineid" => $OrigineId
                    ...
                    if(!is_null($occurence2)){
                        ...
                        if(isset($content[$Tab1][$Tab3]['param']['options'] )){
                            foreach($content[$Tab1][$Tab3]['param']['options'] as $Tab5=> $Tab6){
                                if($Tab5 != ""){
                                    $ExtraIdExtrafieldValue = $Id;
                                    $ValeurExtrafieldValue = $Tab5;
                                    $TexteExtrafieldValue = $Tab6;
                                    $occurence3 = $em->getRepository(Extrafieldsvalues::class)
                                        ->findOneBy(array(
                                            "extraid" => $ExtraIdExtrafieldValue,
                                            "valeur" => $ValeurExtrafieldValue
                                        ));
                                    if(is_null($occurence3)){
                                        $tp_entityextra = new Extrafieldsvalues();
                                        $tp_entityextra->setTexte($TexteExtrafieldValue)
                                                ->setValeur($ValeurExtrafieldValue)
                                                ->setCreatedat(NULL)
                                                ->setUpdatedat(NULL)
                                                ->setDeletedat(NULL)
                                                ->setExtraid($em->getReference('App\Entity\Extrafields', $ExtraIdExtrafieldValue));
                                        $em->persist($tp_entityextra);
                                    }else{
                                        $occurence3->setTexte($TexteExtrafieldValue)
                                            ->setValeur($ValeurExtrafieldValue)
                                            ->setExtraid($em->getReference('App\Entity\Extrafields', $ExtraIdExtrafieldValue));
